...
@section('title', __('messages.schools.edit_school'))
        ...
        <div class="col">
          ...
          <div class="page-pretitle">
            <ol class="breadcrumb breadcrumb-arrows" aria-label="breadcrumbs">
              ...
              <li class="breadcrumb-item active" aria-current="page"><a href="#">{{ __('messages.schools.breadcrumb_edit') }}</a></li>
            </ol>
          </div>
          <h2 class="page-title">
        ...
        <div class="col-12">
          <form class="card" action="{{ route('schools.update', ['school' => $school->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="card-header">
              <h3 class="card-title">{{ __('messages.schools.edit_school_name', ['name' => $school->name]) }}</h3>
            </div>
            <div class="card-body">
              <div class="mb-3">
                ...
                <div>
                  <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                    aria-describedby="emailHelp" placeholder="{{ __('messages.schools.enter_name') }}" value="{{ old('name') ?? $school->name }}"
                    required>
                </div>
                @error('name')
              ...
              <div class="mb-3 mb-0">
                ...
                <textarea rows="5" class="form-control @error('address') is-invalid @enderror" name="address"
                  placeholder="{{ __('messages.schools.enter_address') }}">{{ old('address') ?? $school->address }}</textarea>
                @error('address')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-3">
                ...
                <div id="photosHelp" class="form-text">{{ __('messages.schools.photos_help') }}
                </div>
                @error('photos')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="card-footer text-end">
              ...
              <button type="submit" class="btn btn-primary">{{ __('messages.schools.update') }}</button>
            </div>
          </form>
        </div>
      </div>
